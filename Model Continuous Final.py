...
def ddegrad(s, c, t):
    ...
    if (t>plyt_added): # after plyt_added hours lytic phage is added
        # Xi = Ki*Xs*P - D*Xi - exp(-D*T)*Ki*Xs(t-T)P(t-T) - KiXiPt
    ...
    # Xl = Xl*u*S/(Km+S) + Ki*Xs*Pt - q*exp(-D*T)*Ki*Xs(t-T)Pt(t-T) - -D*Xl + KiXiPt
    g[4] = s[4]*(c[0]*s[0])/(c[5]+s[0]) + c[3]*s[1]*s[5] -c[8]*exp(-c[2]*c[7])*Xllag -c[2]*s[4]
    # Pt = b*q*exp(-D*T)*Ki*Xs(t-T)Pt(t-T)  - Ki*Xs*Pt - Ki*Xl*Pt - Ki*Xi*Pt
